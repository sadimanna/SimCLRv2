...
class SelectiveKernel(nn.Module):
    def __init__(self, in_channels, out_channels, stride, sk_ratio, min_dim=32):
        super().__init__()
        assert sk_ratio > 0.0
        self.main_conv = nn.Sequential(conv3x3(in_channels, 2 * out_channels, stride=stride),
                                       BatchNormRelu(2 * out_channels))
        mid_dim = max(int(out_channels * sk_ratio), min_dim)
        self.mixing_conv = nn.Sequential(conv1x1(out_channels, mid_dim),
                                         BatchNormRelu(mid_dim),
                                         conv1x1(mid_dim, 2 * out_channels))
    ...
    def forward(self, x):
        ...
        return (x * torch.nn.functional.softmax(m, dim=0)).sum(dim=0)
...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        self.skratio = skratio
        # Both self.conv2 and self.downsample layers downsample the input when stride != 1
        self.conv1 = conv1x1(inplanes, width)
        self.bn1 = norm_layer(width)
        ##### Replace with SK ####
        if skratio > 0:
            self.conv2 = SelectiveKernel(width, width, stride=stride, sk_ratio=skratio)
...
class ResNet(nn.Module):
    def __init__(
        ...
        self.layer1 = self._make_layer(block, 64, layers[0], skratio=skratio)
        self.layer2 = self._make_layer(block, 128, layers[1], stride=2, dilate=replace_stride_with_dilation[0], skratio=skratio)
        self.layer3 = self._make_layer(block, 256, layers[2], stride=2, dilate=replace_stride_with_dilation[1], skratio=skratio)
        self.layer4 = self._make_layer(block, 512, layers[3], stride=2, dilate=replace_stride_with_dilation[2], skratio=skratio)
    ...
    def _make_layer(
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            if skratio > 0.0:
                downsample = nn.Sequential(nn.ZeroPad2d((0, 1, 0, 1)),
                                          # kernel_size = 2 => padding = 1
                                          nn.AvgPool2d(kernel_size=2, stride=stride, padding=0),
                                          conv1x1(self.in_planes, planes * block.expansion),
